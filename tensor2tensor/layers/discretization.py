...
def vq_nearest_neighbor(x, means,
  ...
  means_TR = tf.linalg.matrix_transpose(means)
  scalar_prod = tf.matmul(x, means_TR)
...
def vq_body(x,
  ...
  def loss_with_update():
    ...
    x_means_hot_TR = tf.linalg.matrix_transpose(x_means_hot)
    dw = tf.matmul(x_means_hot_TR, x)
...
def gumbel_softmax_discrete_bottleneck(x,
  ...
  updated_ema_count = moving_averages.assign_moving_average(
      ...
      decay,
      zero_debias=False)
  x_means_assignments_TR = tf.linalg.matrix_transpose(x_means_assignments)
  dw = tf.matmul(x_means_assignments_TR, x)
  updated_ema_means = tf.identity(
      moving_averages.assign_moving_average(
          ema_means, dw, decay, zero_debias=False))
  n = tf.reduce_sum(updated_ema_count, axis=-1, keepdims=True)
