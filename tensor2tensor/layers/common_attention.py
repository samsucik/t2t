...
class LshGating(object):
  ...
  def get_gates(self, x):
    ...
    t_group_TR = tf.linalg.matrix_transpose(self.t_group)
    x = tf.matmul(x, t_group_TR) / self.nb_hyperplanes
...
def grouped_attention_multihead(query_antecedent,
  ...
  with tf.variable_scope(
    ...
    k_TR = tf.linalg.matrix_transpose(k)
    logits = tf.matmul(dispatched_q, k_TR)
    ...
    if common_layers.should_generate_summaries() and make_image_summary:
      ...
      if make_image_summary:
        ...
        m_gates_trunc_TR = tf.linalg.matrix_transpose(m_gates_trunc)
        grouping_mask = tf.matmul(
            q_gates_trunc, m_gates_trunc_TR)
        q_trunc = q[:trunc_heads, :trunc_length_q, :] #!!
        k_trunc = kv[:trunc_heads, :, :depth_qk]
        k_trunc_TR = tf.linalg.matrix_transpose(k_trunc)
        logits_trunc = tf.matmul(q_trunc, k_trunc_TR)
        if mask_right:
...
def dot_product_attention_simplified(q,
                          k,
                          v,
                          bias,
                          dropout_rate=0.0,
  ...
  with tf.variable_scope(
    ...
    k_TR = tf.linalg.matrix_transpose(k, name="QKMatMul-k")
    q_named = tf.identity(q, name="QKMatMul-q")

    # logits = tf.matmul(q_named, k_TR, name="QKMatMul")  # [..., length_q, length_kv]
    logits_slices = []
    for head_idx in range(num_heads):
      logits_slice = tf.matmul(q_named[0][head_idx], 
                               k_TR[0][head_idx], 
                               name='QKMatMul-slice{}'.format(head_idx))
      logits_slices.append(logits_slice)

    logits = tf.expand_dims(tf.stack(logits_slices, name='QKMatMul-3d'), 
                            0, name='QKMatMul-4d')
    if bias is not None:
    ...
    # return tf.matmul(weights, v, name="AttMatMul")
    context_slices = []
    for head_idx in range(num_heads):
      context_slice = tf.matmul(weights[0][head_idx], 
                               v[0][head_idx], 
                               name='ContextMatMul-slice{}'.format(head_idx))
      context_slices.append(context_slice)

    context = tf.expand_dims(tf.stack(context_slices, name='ContextMatMul-3d'), 
                            0, name='ContextMatMul-4d')
    return context
...
def _relative_attention_inner(x, y, z, transpose):
  ...
  if transpose:
    y_TR = tf.linalg.matrix_transpose(y)
    xy_matmul = tf.matmul(x, y_TR)
  else:
    xy_matmul = tf.matmul(x, y)
  ...
  if transpose:
    z_TR = linalg.matrix_transpose(z)
    x_tz_matmul = tf.matmul(x_t_r, z_TR)
  else:
...
def dot_product_unmasked_self_attention_relative_2d(
  ...
  with tf.variable_scope(
    ...
    # [batch, num_heads, query_length, memory_length]
    flatten_hw_TR = tf.linalg.matrix_transpose(flatten_hw(k, depth_k))
    logits = tf.matmul(flatten_hw(q, depth_k), flatten_hw_TR)
...
def dot_product_unmasked_attention_local_2d_tpu(
  ...
  with tf.variable_scope(
    ...
    keys = tf.linalg.matrix_transpose(keys)
    logits = tf.matmul(queries, keys_TR)
    if bias is not None:
      logits += bias
...
def masked_relative_local_attention_1d(q,
  ...
  with tf.variable_scope(
    ...
    first_k_TR = tf.linalg.matrix_transpose(first_k)
    first_logits = tf.matmul(first_q, first_k_TR)
    first_logits += (
    ...
    all_rel_logits = tf.concat([unmasked_rel_logits, masked_rel_logits],
                               axis=3)
    rel_k_TR = tf.linalg.matrix_transpose(rel_k)
    all_logits = (
        tf.matmul(rel_tail_q, rel_k_TR) + all_rel_logits)
    # make sure source_pos <= target_pos
...
def multihead_attention(query_antecedent,
  ...
  with tf.variable_scope(name, default_name="multihead_attention",
    ...
    elif attention_type == "dot_product_simplified":
      print("NUM_HEADS", num_heads)
      x = dot_product_attention_simplified(
            q, k, v, bias, dropout_rate, image_shapes,
            save_weights_to=save_weights_to,
            make_image_summary=make_image_summary,
            dropout_broadcast_dims=dropout_broadcast_dims,
...
def scaled_dot_product_attention_simple(q, k, v, bias, name=None):
  ...
  with tf.variable_scope(
    ...
    k_TR = tf.linalg.matrix_transpose(k)
    logits = tf.matmul(q * scalar, k_TR)
    if bias is not None:
      logits += bias
    weights = tf.nn.softmax(logits, name="attention_weights")
