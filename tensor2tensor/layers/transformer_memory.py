...
class TransformerMemory(object):
  ...
  def _address_content(self, x):
    ...
    mem_query = tf.layers.dense(x, self.key_depth,
                                bias_initializer=tf.constant_initializer(1.0),
                                name="mem_query")
    mem_keys_TR = tf.linalg.matrix_transpose(self._norm(mem_keys))
    norm = tf.matmul(self._norm(mem_query), mem_keys_TR)
    mem_keys_TR = tf.linalg.matrix_transpose(mem_keys)
    dot_product = tf.matmul(mem_query, mem_keys_TR)
