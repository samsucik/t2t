...
class ExponentiatedQuadratic(object):
  ...
  def __call__(self, x1, x2):
    ...
    x2_TR = tf.linalg.matrix_transpose(x2)
    square = (x1_squared[:, tf.newaxis] +
              x2_squared[tf.newaxis, :] -
              2 * tf.matmul(x1, x2_TR))
...
class LinearKernel(object):
  ...
  def __call__(self, x1, x2):
    ...
    encoded_x2_TR = tf.linalg.matrix_transpose(encoded_x2)
    dot_product = tf.matmul(encoded_x1, encoded_x2_TR)
...
class BayesianLinearModel(tf.keras.Model):
  ...
  def fit(self, x=None, y=None):
    ...
    # Bayesian updating.
    x_TR = tf.linalg.matrix_transpose(x)
    kernel_matrix = tf.matmul(x_TR, x) / self.noise_variance
